...
const RSVPDisplay = ({ words, currentIndex, chunkSize, pageIndex }) => {
  ...
  const renderContent = () => {
    // Only use red-letter highlighting if it's a single word AND it doesn't contain spaces (not a phrase)
    if (chunkSize === 1 && currentWords[0] && !currentWords[0].includes(' ')) {
